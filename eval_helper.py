...
def performances_just_masks(preds, masks, config, clsname):
    ret_metrics = {}
    preds = np.asarray(preds, dtype=np.float32)  # N x H x W
    masks = np.asarray(masks, dtype=np.float32)  # N x H x W
    ...
    for clsname in clsnames:
        ...
        for fileinfo, pred, mask in zip(fileinfos, preds, masks):
            if fileinfo["clsname"] == clsname:
                preds_cls.append(pred.cpu().data.numpy()[None, ...])
                masks_cls.append(mask.cpu().data.numpy()[None, ...])
